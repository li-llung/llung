...
(function($, z){
	...
	z.messages = {
        ...
        error: function (where, error) {
            ...
            $(where).prepend(z.messages.templates.base.format('issueAlert', 'Please correct the following items:', 'issueAlert', error));
        },
        success: function (where, message, header_text) {
            var message_text = (message === undefined) ? '' : message;
            $(where).prepend(z.messages.templates.base.format('successAlert', header_text, 'successAlert', message_text));
        },
        success_no_header: function (where, message) {
            var message_text = (message === undefined) ? '' : message;
            $(where).prepend(z.messages.templates.base_no_header.format('successAlert', 'successAlert', message_text));
        },
        warning: function (where, message) {
            var message_text = (message === undefined) ? '' : message;
            $(where).prepend(z.messages.templates.warning.format(message_text));
        },
        clear: function(){
            $('.messages').remove();
        }
